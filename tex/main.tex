...
\citep{2020SciPy-NMeth}. \textcolor{red}{These algorithms are efficient methods for finding zeros of systems of non-linear equations. They are based on the Powell hybrid method \cite{powell70}.} It should be noted that in this section we assume that
...
\begin{itemize}
  ...
  communication en-route. \textcolor{red}{Consider also the situation when a
...
the available 2019 demand. \textcolor{red}{APPENDIX}~\ref{apx:parameters} provides further details on the
...
\textcolor{red}{There is a large discrepancy between the expected survival
...
\textcolor{red}{
\subsection{Comparing Expected \& Simulated Survival Measures}
...
solution such that will ensure that the expected response time is above $L$,
possibly as low as $L + \epsilon$, but will not attempt to push the expected
...
improve runtime efficiency. However, as discussed in Section~\ref{sec:survival},
maximising expected survival of all patients can lead to a fairer allocation of
resources, with difficult to reach patients being given equal consideration to
closer, easier to reach patients. When considering the variability of travel
times in the optimisation process, difficult to reach patients may be ignored, indistinguishable from patients who happen to sample a response time at the
extreme tail of a given distribution curve.}

\textcolor{red}{
Therefore, due to this interpretation of fairness of allocations, and due to the
naive choice of travel time distributions, including difficulties in being able
to validate such a model and the choices made, we consider only the MESLMHPHF
